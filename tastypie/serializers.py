...
for integer_type in [int]:
    _SIMPLETYPES[integer_type] = _NUM

for string_type in [str]:
...
class Serializer(object):
    ...
    def to_simple(self, data, options):
        ...
        if stype == _DICT:
            ...
            return {key: to_simple(val, options) for key, val in iter(data.items())}
        if stype == _STR:
        ...
        if stype == _BUNDLE:
            to_simple = self.to_simple
            return {key: to_simple(val, options) for key, val in iter(data.data.items())}
        if stype == _DATETIME:
